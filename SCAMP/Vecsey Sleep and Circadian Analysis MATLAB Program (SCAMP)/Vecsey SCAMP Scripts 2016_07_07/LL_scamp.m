...
%% 1 minute bins    
    filenames_1min = dir(onePath);
    filenames_30min = dir(thirtyPath);

    theList = {onePath thirtyPath};
    last = '';
    
    first = [];
    for i=1:length(filenames_1min)
        current=filenames_1min(i).name;
...
%% thirty minute bins    
    for i=first:length(filenames_30min)
        current = filenames_30min(i).name;
        index = strfind(current,'C');
        if isempty(first) && ~isempty(index)
            first = i;
        end
...
%         theList = [theList {last}]; 
%     end
%         last = name;
% end % i
        
%    theList = theList(2:end);
...
% If you prefer to do this by looking at activity, uncomment two lines for running ralf instead.
    ...
    for i=1:length(selected)
       ...
       [oneObject,thirtyObject]=myDamLoad(board,onePath,thirtyPath);
