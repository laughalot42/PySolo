...
class cfgPanel(wx.Panel):
    ...
    def videosWidget(self):
        self.scrolledThumbs = SW.scrollingWindow(self)      # create scrolling window with thumbnails for each monitor
...
#            gbl.source = 'Webcam1'                     # only one webcam is supported, but multiple monitors can use it
    ...
    def onChangeThumbFPS(self, event):
        ...
        if input <> self.thumbFPS:
            ...
            for mon_ID in range(1, gbl.monitors+1):
                ...
                if self.scrolledThumbs[mon_ID].monitorPanel.playTimer.IsRunning():
    ...
    def changeConfig(self):             # TODO doesn't erase consoles
        ...
        if self.shouldSaveCfg:
            answer = self.TopLevelParent.config.Q_wantToSave()        # save this config before loading new one?
            if answer == wx.ID_YES:
        ...
        self.scrolledThumbs.refreshThumbGrid()              # update and start the thumbnails window
    ...
    def onStartAcquisition(self, event=None):
        ...
        """
        if self.shouldSaveCfg:
            answer = self.TopLevelParent.config.Q_wantToSave()            # ask about saving configuration
            if answer == wx.ID_YES:
                cfg.cfg_nicknames_to_dicts()
        ...
        for gbl.mon_ID in range(1, gbl.monitors + 1):
            self.scrolledThumbs.trkdMonsList[gbl.mon_ID].start()           # start tracking
    ...
    def onMonitorVideos(self, event):
        self.Parent.notebookPages[0].scrolledThumbs.clearGrid()
        self.Parent.notebookPages[0].scrolledThumbs.refreshThumbGrid()
